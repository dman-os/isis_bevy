...
pub fn butler(
    ...
    mut strategies: ParamSet<(
        Query<(Entity, &CAS, &FlockStrategy), Added<CAS>>, // added
        Query<(Entity, &CAS)>,                             // all
    )>,
    ...
    for (strategy_entt, _param, strategy) in strategies.p0().iter_mut() {
        let members = flocks
    ...
    for (flock_entt, members) in member_changes.iter() {
        if let Ok((strategy_entt, _param)) = strategies.p1().get_mut(flock_entt) {
            for boid_entt in members.iter() {
                let boid_entt = *boid_entt;
                let (mut directive,) = crafts
                    .get_mut(boid_entt)
...
pub fn update(
    ...
    flocks: Query<&FlockMembers>,
    crafts: Query<(&GlobalTransform, &Velocity)>,
) {
